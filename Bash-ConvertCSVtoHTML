...
tr:nth-child(even) {background: #E7F7ED}  // changing-colors
tr:nth-child(odd) {background: #CBEED9}   // changing-colors
...
#myInput {
  ...
  border:2px solid #009E66;  // changing-colors
  margin-bottom: 5px;
}
...
#myTable tr:first-child {
  ...
  font-weight: bold;
       ...
       echo '<h2 style="text-align:left;background-color:White;color:#009E66;">'$MY_HEADER'</h2>'
...
cat /dev/stdin | sed '/^$/d' | while IFS=$IFS_CHAR read -ra line ; do
	...
	if [ $lineno == 1 ]; then
	     echo '<tr style ="background: #aaaaaa;font-weight: bold;text-align:left; vertical-align:middle; font-size:14px">'
	elif [ $(($lineno%2)) == 0 ]; then
		 echo '<tr style ="background: #E7F7ED ;text-align:left; vertical-align:middle; font-size:14px">'
	else
		echo '<tr style ="background: #CBEED9;text-align:left; vertical-align:middle; font-size:14px">'
	fi
